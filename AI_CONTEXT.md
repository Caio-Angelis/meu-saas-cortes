...
**Verificação após alterações:** com o venv ativo e dependências de dev instaladas (`pip install -r requirements-dev.txt`), rode **`pytest`** na raiz (config em `[tool.pytest.ini_options]` no `pyproject.toml`, `testpaths = ["tests"]`, `addopts = "-q"`). Em jul/2026 a suíte tem **~162 testes** (+5 split em `test_subtitle_burner_split.py`; `test_resolve_hit_sfx_prefers_ball_mp3` exige `assets/ball.mp3`) e cobre lógica pura e integrações leves **sem** chamar Groq, FFmpeg ou yt-dlp na maior parte dos casos — ideal para checar regressões rápidas antes de um processamento completo.

**Checklist melhorias (worktree `checklist-melhorias`):** itens **1.1–1.6** — `faster-whisper==1.2.1` no `.venv`; config `TRANSCRIBE_BACKEND` / `LOCAL_WHISPER_*`; `local_whisper.py` + atalho em `transcriber.py` (`TRANSCRIBE_BACKEND=local`, fallback Groq); `.env.example` documentado. **1.6** E2E OK: log `Transcrição LOCAL (faster-whisper na GPU)` com `temp/checklist_1.6_sample_25s.mp4` → `resultados/1_checklist_1.6_sample_25s.mp4`; pytest OK. **2A.1** — estilo ASS TikTok em `ass_builder.write_tiktok_ass_from_srt`: negrito + BorderStyle contorno (sem caixa opaca) + outline 4 + sombra 1. **2A.2** — verificação visual OK (`temp/check_2A2_frame.png`: amarelo bold + contorno preto, sem caixa retangular); pytest OK. **2B.1** — `assets/fonts/Montserrat-Bold.ttf` (SIL OFL, ~445KB) empacotado. **2B.2** — `TIKTOK_SUBTITLE_FONT` / `FONTS_DIR` em `config.py`. **2B.3** — `fontsdir` no filtro `subtitles` em `_prepare_scale_crop_overlay_vf` (`subtitle_burner.py`). **2B.4** — override de `fonte` `None`/`""`/`"Arial"` → `TIKTOK_SUBTITLE_FONT` no início de `_prepare_scale_crop_overlay_vf`. **2B.5** — verificação visual OK (`temp/check_2B5_frame.png`: Montserrat Bold geométrica, não Arial); pytest OK. **2C.1** — `SUBTITLE_KARAOKE` / `SUBTITLE_KARAOKE_HIGHLIGHT` em `config.py` (default ligado, highlight `#FFE000`). **2C.2** — `write_tiktok_ass_karaoke_from_srt` + `_hex_to_ass` em `ass_builder.py` (tags `{\k}` por palavra). **2C.3** — import de `write_tiktok_ass_karaoke_from_srt` em `subtitle_burner.py`. **2C.4** — branch em `_prepare_scale_crop_overlay_vf`: se `SUBTITLE_KARAOKE` → karaoke ASS, senão plain. **2C.5** — verificação visual OK (`temp/check_2C5_{a,b,c}.png`: branco→amarelo avança na mesma linha); pytest OK. **3.1** — `_clip_uses_gpu_encoder` retorna `USE_GPU_CLIP_ENCODE` (todos os clipes na GPU quando ligado). **3.1b** — `test_gpu_on_all_clips_use_gpu` substitui os testes da regra antiga; `test_gpu_off_never` mantido; pytest verde. **3.2** — NVENC preset `p6`/`cq 19` + bf/lookahead em `gpu_clip_encoder_ffmpeg_args()`. **3.3** — `.env` `CLIP_ENCODE_PARALLEL_GPU=4` (RTX 16 GB); `.env.example` comentário alinhado. **3.4** — E2E multi-clipe NVENC OK: `VIRAL_CLIPS_COUNT=3` em `ytdl_514bda5a94f24646.mp4` → 5 MP4 em `resultados/` com tag `encoder=… h264_nvenc`; log `CPU/GPU conforme configuração`; pytest OK. **4A.1** — import de `gpu_clip_encoder_ffmpeg_args` em `batalha_ffmpeg.py`. **4A.2** — `encode_simulation_to_silent_mp4` usa `*gpu_clip_encoder_ffmpeg_args()` no `cmd` (NVENC com fallback libx264). **4A.3** — smoke `encode_simulation_to_silent_mp4` (1s Tamanho) → `resultados/task43_batalha_nvenc_smoke.mp4` com tag `encoder=… h264_nvenc`; pytest OK. **4B.1** — import de `gpu_clip_encoder_ffmpeg_args` em `historia_pipeline.py`. **4B.2** — `_mux_cena_video_audio` usa `*gpu_clip_encoder_ffmpeg_args()` no `cmd` (NVENC com fallback libx264). `requirements.txt` **não** foi alterado. **4B.3** — smoke `_mux_cena_video_audio` (vídeo curto + wav) → `resultados/task4B3_historia_nvenc_smoke.mp4` com tag `encoder=… h264_nvenc`; pytest OK. **5.1** — `_smooth_samples_ema` em `focal_crop.py` (antes de `_speaker_timeline_crop_segments`). **5.2** — `_smooth_samples_ema(samples, alpha=0.35)` aplicado após `_stabilize_speaker_changes_min_interval` em `_speaker_timeline_crop_segments`. **5.3** — headroom em `_clamp_crop_xy`: `y` sobe o alvo em ~8% de `out_h` (`- out_h * 0.08`). **5.4** — verificação visual OK (`temp/check_5.4_{a,b,c}.png` de `resultados/1_checklist_1.6_sample_25s.mp4`): falante com headroom (rosto acima do centro); frames 8s/12s sem salto brusco; pytest OK. **6A.1** — `_voiced_intervals` / `_is_voiced` em `focal_crop.py` (FFmpeg `silencedetect`, invertido; ainda não ligado aos callers). **6A.2** — parâmetro `voiced: list[tuple[float, float]] | None = None` na assinatura de `_speaker_timeline_crop_segments`. **6A.3** — gate de silêncio no corpo: se `voiced` e `_is_voiced(t, voiced)` falso, mantém `last_speaker_i`; com `voiced=None` o comportamento permanece igual. **6A.4** — `compute_crop_plan` calcula `voiced = _voiced_intervals(...)` quando há clip e passa `voiced=voiced` para `_speaker_timeline_crop_segments`. **6A.5** — verificação visual/E2E do áudio-gate: `temp/checklist_1.6_sample_25s.mp4` → `resultados/1_checklist_1.6_sample_25s.mp4` + `temp/check_6A.5_{a,b,c}.png`; sample é **1 falante** (sem 2 pessoas em `temp/`/`resultados/` para validar troca); pipeline OK, crop/headroom estáveis em b↔c; `_voiced_intervals` retorna intervalos; pytest OK. **6B.1–6B.5** — `SMART_CROP_SPLIT_ENABLED` default **ligado** (`"1"`); plano `mode: "split"` + render FFmpeg `vstack` em `subtitle_burner` (`-filter_complex` → `[vout]`; encode split sempre CPU). **7.1** — `VISUAL_*` em `config.py`. **7.2** — grade/barra/watermark em `cut_and_burn_subtitles` (`subtitle_burner.py`). **8.1–8.2** — `_MAX_TRANSCRIPT_CHARS=24_000` + amostragem uniforme em `viral_analyzer._build_transcript_text`. **9.1–9.3** — retry de TimeoutError Edge; `_synthesize_jobs` (Kokoro→Edge) em `tts_dubber.py`. **9.4** — pytest OK; E2E dub (`--dub-pt`) não exercitado nesta sessão (checklist permite marcar sem testar). **10.1–10.3** — `--concurrent-fragments` + stub `YTDLP_CONCURRENT_FRAGMENTS` no `.env.example`. **11.2** — `limpar_temp.sh` (mtime +2d). **11.3** — SQLite `PRAGMA journal_mode=WAL` em `store._connect`. **7.3** — visual OK (grade/vignette/watermark); barra usa `thickness=fill` (fix do shadowing `t=fill`). **11.1** — loudnorm no `af`. **12.1–12.3** — TikTok upload URL + botão GUI (sem auto-post). **Checklist melhorias FECHADO e removido (2026-07-15):** fases 0–13 concluídas; arquivos `CHECKLIST_MELHORIAS.md` e `checklist.md` excluídos. Relatórios em `.sdd-briefs/` (`task-6B.4-report.md`, `task-13.2-report.md`). **8.3** amostragem uniforme OK (E2E moments longo pós-8.2 não refeito). **10.4** download URL OK. **12.4** botão TikTok código+smoke OK (clique humano/post real opcional). **13.1** pytest 157→161 OK (+5 split). **13.2 E2E 2P fechado (2026-07-15):** fonte `temp/e2e_13_2_src/e2e_13_2_two_speakers_50s.mp4` → `temp/e2e_13_2_out/1_e2e_13_2_two_speakers_50s.mp4`; local faster-whisper; crop `mode=dynamic` L↔R; `h264_nvenc`; barra/legendas OK; TikTok GUI skip; ver `.sdd-briefs/task-13.2-report.md`. **6B.4/6B.5 IMPLEMENTED (2026-07-15):** split renderer + flag default on.
...
- **Tema «Editing Bay»:** shell custom em `app/gui/studio_theme.py` + assets em `assets/gui/` (fundo atmosférico, logo, botões PNG, sidebar); paleta ink/musgo (sem cyan/sv-ttk); header com marca **STUDIO CORTES**; seções com borda `EDGE`; CTA **Gerar clipes** e **Postar no TikTok** com imagens; `Panedwindow` log/resultados; barra de progresso musgo.
...
- **Pós-processo:** tabela da última execução (nome + duração via **ffprobe** em thread); botões **copiar legenda** (`.txt` TikTok), **copiar caminho** do MP4, **Postar no TikTok (selecionado)** (clipboard + pasta + upload URL; sem auto-login), **copiar todas as legendas**, **exportar .zip** (`app/gui_export.py` — inclui MP4, `.txt` ao lado e `LEIA-ME_POSTAGEM.txt`).
- **Seção Melhorias (aba Cortes):** toggles de `TRANSCRIBE_BACKEND` (local/groq), karaokê, grade/vignette, barra de progresso, smart crop, encode GPU, preferir Kokoro na dublagem, watermark `@`; botão **Limpar temp/ (+2 dias)**. Ao gerar, a GUI muta `app.core.config.*` (flags eram só `.env`). Split 2P permanece off.
...
| Smart crop | `SMART_CROP_ENABLED`, `SMART_CROP_FRAME_SAMPLES`, `SMART_CROP_SPEAKER_FPS`, `SMART_CROP_MIN_CHANGE_INTERVAL_SEC`, `SMART_CROP_MEDIAPIPE_GPU`, `SMART_CROP_MEDIAPIPE_GPU_FORCE`, `SMART_CROP_SPLIT_ENABLED` (flag 6B; **default on** — plano `mode: "split"` + render `vstack` via `-filter_complex`; encode split sempre CPU), `BLAZEFACE_TFLITE_SHA256` (opcional) |
...
| `subtitle_burner.py` | `burn_subtitles` (input já cortado) e **`cut_and_burn_subtitles`** (fonte + intervalo: um encode). Legendas ASS no rodapé com margem ampliada (`SUBTITLE_BOTTOM_MARGIN_MULTIPLIER`, `SUBTITLE_RAISE_FACTOR`) para não cobrir nome do perfil no TikTok. Fallback CPU **sem** hwupload se GPU falhar. **Split (`mode: "split"`):** `_build_split_vstack_graph` + `-filter_complex` → `[vout]`; `cut_and_burn` injeta `vf_cut`/`VISUAL_*` nos pads; encode do split **sempre CPU**. |
| `focal_crop.py` | Plano de crop; **`compute_crop_plan(..., clip_start, clip_end)`** amostra só o trecho; delegate MediaPipe **GPU por padrão** com driver NVIDIA no Linux (ou `SMART_CROP_MEDIAPIPE_GPU=1`); CPU em AMD+Mesa salvo `SMART_CROP_MEDIAPIPE_GPU_FORCE=1`; `SMART_CROP_MEDIAPIPE_GPU=0` força CPU; crop dinâmico para falante com 2+ rostos; com `SMART_CROP_SPLIT_ENABLED` + clip + 2 rostos afastados (>35% largura) retorna **`mode: "split"`** via **`_two_people_centers`** (+ `src_w`/`src_h`; render em `subtitle_burner`); **`_smooth_samples_ema`** (EMA em cx/cy; aplicada após stabilize em `_speaker_timeline_crop_segments`, α=0.35); **`_clamp_crop_xy`** com headroom ~8% (`- out_h * 0.08`); **`_voiced_intervals`** / **`_is_voiced`** (FFmpeg silencedetect → intervalos de voz); **`compute_crop_plan`** passa `voiced=` para **`_speaker_timeline_crop_segments`** quando há clip; **`_speaker_timeline_crop_segments(..., voiced=None)`** usa gate de silêncio no corpo (`_is_voiced`); com `voiced=None` sem mudança; **`_opencv_file_capture()`** tenta `CAP_FFMPEG` primeiro. |
...
| `test_subtitle_burner_split.py` | split `vstack` filter_complex; static/dynamic `-vf` |
