...
3. **GPU** — últimos clipes da lista tendem a usar encoder GPU quando há clipes suficientes e `USE_GPU_CLIP_ENCODE`; **semáforos** limitam quantos encodes GPU/CPU rodam ao mesmo tempo (padrão **3** encodes NVENC em paralelo com NVIDIA no Linux); falha de driver cai para CPU nos pontos de encode. No Linux + **NVIDIA**, o padrão é **NVENC** (`h264_nvenc`); no Linux + Mesa/AMD, **VA-API** (`h264_vaapi`) e heurística de `renderD*` para iGPU+dGPU.
