...
Documento de contexto para outra IA ou desenvolvedor entender o projeto **sem ler o código inteiro**. Última revisão: **2026-07-14**, alinhada ao código em `app/`, `main.py`, `gui.py` e `web_main.py`.
...
| Smart crop | `SMART_CROP_ENABLED`, `SMART_CROP_FRAME_SAMPLES`, `SMART_CROP_SPEAKER_FPS`, `SMART_CROP_MIN_CHANGE_INTERVAL_SEC`, `SMART_CROP_MEDIAPIPE_GPU`, `SMART_CROP_MEDIAPIPE_GPU_FORCE`, `SMART_CROP_SPLIT_ENABLED` (flag 6B; **default off** — plano `mode: "split"` existe, render 6B.4 adiado), `BLAZEFACE_TFLITE_SHA256` (opcional) |
